  ...
  return (
    <div className="metric-card">
      ...
      <div className="metric-value">
          ...
          {status ? <i className={"status-pill " + status.toLowerCase()} data-comment-anchor="31b4cd0f63-i-62-21">{status}</i> : null}
        </div>
      }
...
/* ── Leaderboard ── */
function Leaderboard({ title, data, filter, setFilter, mode, limit = 5, onMore, moreLabel }) {
  ...
  const valOf = mode === "sov" ? (r) => r.count / totalCount : (r) => r.visibilityRate;
  const bandOf = mode === "vis" ? (v) => band(v, 0.3, 0.2)
    : mode === "sov" ? (v) => (v >= 0.15 ? "High" : v >= 0.08 ? "Medium" : "Low")
    : (v) => v > 0.4 ? "High" : v >= 0.2 ? "Medium" : "Low";
  const unit = mode === "vis" ? "checks" : mode === "sov" ? "mentions" : "prompts";
  return (
    <div className="panel">
      ...
      <div className="lb-list">
        ...
        <div key={r.name + i} className={"lb-row" + (r.isTarget ? " you" : "")}>
            ...
            <Track value={r.count / maxCount} tone={bandOf(valOf(r))} />
            <b>{pct(valOf(r))}</b>
          </div>
        )}
      </div>
      {onMore ? <button className="text-cta" onClick={onMore}>{moreLabel} <Icon name="arrow" size={13} /></button> : null}
    </div>);
...
/* ── Semicircular visibility gauge (barometer) ── */
function Gauge({ value }) {
  const cx = 100, cy = 100, r = 78, sw = 16;
  const toRad = (v) => ((180 - v * 1.8) * Math.PI) / 180;
  const pt = (v, rad = r) => { const a = toRad(v); return [cx + rad * Math.cos(a), cy - rad * Math.sin(a)]; };
  const seg = (a, b) => {
  ...
  return (
    <svg className="gauge-svg" viewBox="-8 -4 216 126" role="img" aria-label={`Visibility score ${value} of 100`}>
      ...
      {ticks.map((t) => { const [tx, ty] = pt(t, r + 16); return <text key={t} className="gtick" x={tx} y={ty + 3.5} textAnchor="middle">{t}</text>; })}
...
Object.assign(window, { pct, signed, shortSurface, band, Icon, Badge, Track, RankCell, BestBadge, MetricCard, PanelHead, Leaderboard, VisibilityRadar, Gauge });