...
const INSIGHT_CATEGORIES = ["Core General", "Product / Brand"];
...
export async function POST(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  // ── (2) INSIGHT REGEN: ask Search "why not recommended" for missing prompts ──
  const insightTargets = report.queries.filter((query) => {
    if (!INSIGHT_CATEGORIES.includes(query.category)) return false;
    const r1 = report.runs.find((r) => r.queryId === query.id && r.surface === "gemini_search" && r.runNumber === 1);
    if (!r1 || isErr(r1.rawAnswer)) return false;
    if (r1.missingInsight) return false; // already has a Search insight (e.g. near-me just re-run)
    const rank = r1.mentions.find((m) => m.isTarget)?.rank ?? null;
    return !(rank && rank <= 5); // only when missing / ranked outside top 5
  });
  let insightsAdded = 0;
  await runPool(insightTargets, CONCURRENCY, async (query) => {
    const r1 = report.runs.find((r) => r.queryId === query.id && r.surface === "gemini_search" && r.runNumber === 1);
    if (!r1) return;
    try {
      const answer = await withRetry(() =>
        diagnoseGeminiMissingRecommendation({ company, location, query, surface: "gemini_search", originalAnswer: r1.rawAnswer })
      );
      r1.missingInsight = { question: `Why did you not recommend ${company.name}?`, answer, createdAt: new Date().toISOString() };
      insightsAdded += 1;
    } catch {
